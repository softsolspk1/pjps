  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch the specific review assignment for this user and article
        ...
        <div className="space-y-10">
          <section className="bg-slate-900 text-white rounded-3xl p-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              <span className="inline-block px-4 py-1 bg-blue-600/20 text-blue-400 rounded-full text-[10px] uppercase font-black mb-4 tracking-widest border border-blue-500/20">
                {isInvited ? "Scholarly Invitation" : "Peer Review Assignment"}
              </span>
              ...
              {isInvited && (
                <div className="flex flex-col gap-6 p-8 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-sm">
                   <div className="flex items-center gap-4 text-blue-400">
                      <ShieldCheck size={24} />
                      <p className="text-sm font-bold uppercase tracking-widest">Expert Invitation Response Required</p>
                   </div>
                   <p className="text-slate-300 text-sm leading-relaxed">
                      You have been selected as an expert reviewer for this research. Please review the title and abstract above to confirm your availability and lack of conflict of interest.
                   </p>
                   <div className="flex gap-4">
                      <button 
                        onClick={() => handleResponse('ACCEPTED')}
                        disabled={submitting}
                        className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-xl font-bold uppercase tracking-widest text-xs transition-all flex items-center gap-2"
                      >
                      ...
                      <button 
                        ...
                        disabled={submitting}
                        className="bg-white/10 hover:bg-white/20 text-white px-8 py-3 rounded-xl font-bold uppercase tracking-widest text-xs transition-all"
                      >
                         Decline
                      </button>
                   </div>
