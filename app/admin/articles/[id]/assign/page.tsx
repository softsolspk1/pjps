...
export default function AssignReviewerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="flex justify-center items-center h-[60vh] text-slate-400 font-bold">Initializing Expert Registry...</div>;
                ...
                <div key={reviewer.id} className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-white border border-slate-100 rounded-2xl hover:border-blue-200 hover:shadow-md transition-all group">
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900 group-hover:text-blue-700 transition-colors">{reviewer.name}</h3>
                  ...
                  <div className="mt-6 md:mt-0 md:ml-8">
                    ...
                    >
                      {assigning === reviewer.id ? <Loader2 className="animate-spin" size={16} /> : <><UserPlus size={16} /> Assign to Peer Review</>}
                    </button>
                  </div>
                </div>
              ))
