...
export async function POST(req: Request) {
  try {
    ...
    if (!session || !["ADMIN", "EDITOR_IN_CHIEF", "FINANCE_ADMIN"].includes(userRole)) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    const { reviewId, amount, reference, chequeUrl } = await req.json();

    // Determine final payment status based on role
    // FINANCE_ADMIN only creates a PENDING reward
    // ADMIN or EDITOR_IN_CHIEF can create a PAID reward or approve a PENDING one
    const targetStatus = (userRole === "ADMIN" || userRole === "EDITOR_IN_CHIEF") ? "PAID" : "PENDING";
    ...
    // Only Send Disbursement Notification Email if status is now PAID
